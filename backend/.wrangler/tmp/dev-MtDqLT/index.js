...
// .wrangler/tmp/bundle-4UJ6bo/checked-fetch.js
...
// .wrangler/tmp/bundle-4UJ6bo/strip-cf-connecting-ip-header.js
...
function success(c, data, message, meta, status = 200) {
  const body = {
    success: true,
    data,
    message,
...
function created(c, data, message = "Data berhasil dibuat") {
  return success(c, data, message, void 0, 201);
}
...
__name(error, "error");
function notFound(c, message = "Data tidak ditemukan") {
...
// src/api/v1/master/cabang.ts
var app = new Hono2();
var cabang_default = app;

// src/api/v1/master/gudang.ts
var app2 = new Hono2();
var gudang_default = app2;

// src/api/v1/master/user.ts
var app3 = new Hono2();
var user_default = app3;

// src/api/v1/master/vendor.ts
var app4 = new Hono2();
var vendor_default = app4;

// src/api/v1/master/kategori-asset.ts
var app5 = new Hono2();
var kategori_asset_default = app5;

// src/api/v1/master/lokasi-asset.ts
var app6 = new Hono2();
var lokasi_asset_default = app6;

// src/api/v1/master/jenis-maintenance.ts
var app7 = new Hono2();
var jenis_maintenance_default = app7;
...
var Socket = /* @__PURE__ */ __name(class {
  ...
  */
  ...
  onConnError(error2) {
    if (this.hasLogger())
      this.log("transport", "error", error2);
...
function isAuthRefreshDiscardedError(error2) {
  return isAuthError(error2) && error2.name === "AuthRefreshDiscardedError";
}
...
var GoTrueClient = class {
    ...
    try {
      ...
      if (expiresWithMargin) {
        if (this.autoRefreshToken && currentSession.refresh_token) {
          ...
          if (error2) {
            if (isAuthRefreshDiscardedError(error2)) {
              this._debug(debugName, "refresh discarded by commit guard", error2);
            } else {
              this._debug(debugName, "refresh failed", error2);
            }
          }
        }
  ...
  async _startPasskeyRegistration() {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error2 } = await _request(this.fetch, "POST", `${this.url}/passkeys/registration/options`, {
          headers: this.headers,
          jwt: session.access_token,
          body: {}
        });
  ...
  async _verifyPasskeyRegistration(params) {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error2 } = await _request(this.fetch, "POST", `${this.url}/passkeys/registration/verify`, {
          headers: this.headers,
          jwt: session.access_token,
          body: {
  ...
  async _startPasskeyAuthentication(params) {
    ...
    assertPasskeyExperimentalEnabled(this.experimental);
    try {
      const { data, error: error2 } = await _request(this.fetch, "POST", `${this.url}/passkeys/authentication/options`, {
        headers: this.headers,
        body: {
  ...
  async _verifyPasskeyAuthentication(params) {
    assertPasskeyExperimentalEnabled(this.experimental);
    try {
      const { data, error: error2 } = await _request(this.fetch, "POST", `${this.url}/passkeys/authentication/verify`, {
        headers: this.headers,
        body: {
          challenge_id: params.challengeId,
          credential: params.credential
  ...
  async _listPasskeys() {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error2 } = await _request(this.fetch, "GET", `${this.url}/passkeys`, {
          headers: this.headers,
          jwt: session.access_token,
  ...
  async _updatePasskey(params) {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error2 } = await _request(this.fetch, "PATCH", `${this.url}/passkeys/${params.passkeyId}`, {
          headers: this.headers,
          jwt: session.access_token,
  ...
  async _deletePasskey(params) {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { error: error2 } = await _request(this.fetch, "DELETE", `${this.url}/passkeys/${params.passkeyId}`, {
          headers: this.headers,
          jwt: session.access_token,
          noResolveJson: true
        });
...
async function authMiddleware(c, next) {
  ...
  const supabase2 = getSupabase(c.env);
  try {
    const { data: { user }, error: authError } = await supabase2.auth.getUser(token);
    if (authError || !user) {
      return unauthorized(c, "Token tidak valid");
    }
    const { data: userData, error: userError } = await supabase2.from("users").select("*, roles(*)").eq("email", String(user.email)).single();
...
// src/midlware/audit.ts
async function logAudit(c, data) {
  const supabase2 = getSupabase(c.env);
  const user = c.get("user");
  const ip = c.req.header("CF-Connecting-IP") || c.req.header("x-forwarded-for");
  try {
    await supabase2.from("audit_log").insert({
...
asset.get("/", async (c) => {
  ...
  let query = supabase2.from("asset").select("*, kategori_asset!inner(*), cabang!inner(*), lokasi_asset(*)", { count: "exact" });
  ...
  if (id_cabang)
    query = query.eq("id_cabang", id_cabang);
  if (id_kategori)
    query = query.eq("id_kategori", id_kategori);
  if (search) {
...
asset.get("/:id", async (c) => {
  ...
  const { data, error: error2 } = await supabase2.from("asset").select("*, kategori_asset(*), cabang(*), gudang(*), lokasi_asset(*), vendor(*), asset_foto(*), created_by(*), updated_by(*)").eq("id", id).is("deleted_at", null).single();
  if (error2 || !data)
    return notFound(c, "Asset tidak ditemukan");
...
asset.post("/", async (c) => {
  ...
  const { data: cabang } = await supabase2.from("cabang").select("kode").eq("id", body.id_cabang).single();
  if (!cabang)
    return validationError(c, "Cabang tidak valid");
  const tahun = (/* @__PURE__ */ new Date()).getFullYear();
  const { count } = await supabase2.from("asset").select("*", { count: "exact", head: true }).like("kode_asset", `AST-${cabang.kode}-${tahun}-%`);
  const urutan = (count || 0) + 1;
  const kodeAsset = `AST-${cabang.kode}-${tahun}-${String(urutan).padStart(5, "0")}`;
  const { data, error: insertError } = await supabase2.from("asset").insert({
  ...
  if (insertError)
    return validationError(c, insertError.message);
  logAudit(c, {
...
asset.put("/:id", async (c) => {
  ...
  const { id } = c.req.param();
  const body = await c.req.json();
  const { data: existing } = await supabase2.from("asset").select("*").eq("id", id).single();
  if (!existing)
    return notFound(c, "Asset tidak ditemukan");
  const { data, error: updateError } = await supabase2.from("asset").update({ ...body, updated_by: user.id, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select("*").single();
  if (updateError)
    return validationError(c, updateError.message);
  logAudit(c, {
...
asset.delete("/:id", async (c) => {
  ...
  if (!existing)
    return notFound(c, "Asset tidak ditemukan");
  const { error: error2 } = await supabase2.from("asset").update({ deleted_at: (/* @__PURE__ */ new Date()).toISOString(), updated_by: user.id }).eq("id", id);
  if (error2)
    return validationError(c, error2.message);
  logAudit(c, {
...
asset.post("/:id/generate-qr", async (c) => {
  const supabase2 = getSupabase(c.env);
  const { id } = c.req.param();
  const { data: asset2 } = await supabase2.from("asset").select("*").eq("id", id).single();
  if (!asset2)
    return notFound(c, "Asset tidak ditemukan");
  const qrData = JSON.stringify({
    kode: asset2.kode_asset,
    nama: asset2.nama,
    id: asset2.id
  });
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(qrData)}`;
  await supabase2.from("asset").update({ qr_code_url: qrUrl }).eq("id", id);
...
asset.get("/:id/history", async (c) => {
  ...
  const { data, error: error2, count } = await supabase2.from("asset_history").select("*, users!inner(nama, email)", { count: "exact" }).eq("id_asset", id).order("created_at", { ascending: false }).range((Number(page) - 1) * Number(perPage), Number(page) * Number(perPage) - 1);
  if (error2)
    return c.json({ success: false, error: error2.message }, 500);
  return success(c, data, void 0, {
    page: Number(page),
...
function generateNomorPR(_env, tahun, urutan) {
  return `PR-${tahun}-${String(urutan).padStart(5, "0")}`;
}
__name(generateNomorPR, "generateNomorPR");
function generateNomorTicket(tahun, urutan) {
  return `TKT-${tahun}-${String(urutan).padStart(5, "0")}`;
}
__name(generateNomorTicket, "generateNomorTicket");
function generateNomorMutasi(tahun, urutan) {
  return `MTS-${tahun}-${String(urutan).padStart(5, "0")}`;
}
__name(generateNomorMutasi, "generateNomorMutasi");

// src/api/v1/asset/mutasi.ts
var mutasi = new Hono2();
...
mutasi.get("/", async (c) => {
  ...
  let query = supabase2.from("asset_mutasi").select("*, asset!inner(kode_asset, nama), cabang_asal:cabang!id_cabang_asal(*), cabang_tujuan:cabang!id_cabang_tujuan(*), creator:users!created_by(nama)", { count: "exact" });
  if (["KCB", "PCB"].includes(roleCode) && user.id_cabang) {
...
mutasi.post("/", async (c) => {
  ...
  const { data: asset2 } = await supabase2.from("asset").select("*").eq("id", body.id_asset).single();
  if (!asset2)
    return notFound(c, "Asset tidak ditemukan");
  if (asset2.status !== "aktif")
    return validationError(c, "Hanya asset aktif yang bisa dimutasi");
  const tahun = (/* @__PURE__ */ new Date()).getFullYear();
  const { count } = await supabase2.from("asset_mutasi").select("*", { count: "exact", head: true }).like("nomor_mutasi", `MTS-${tahun}-%`);
  ...
  const mutasiData = {
    ...
    tipe: body.tipe,
    alasan: body.alasan,
    tanggal_mutasi: body.tanggal_mutasi || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
    status: "diajukan",
    created_by: user.id
  };
  if (asset2.id_cabang)
    mutasiData.id_cabang_asal = asset2.id_cabang;
  if (asset2.id_gudang)
    mutasiData.id_gudang_asal = asset2.id_gudang;
  if (body.id_cabang_tujuan)
    mutasiData.id_cabang_tujuan = body.id_cabang_tujuan;
  if (body.id_gudang_tujuan)
    mutasiData.id_gudang_tujuan = body.id_gudang_tujuan;
  const { data, error: error2 } = await supabase2.from("asset_mutasi").insert(mutasiData).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("asset_history").insert({
    id_asset: body.id_asset,
    tipe_event: "mutasi",
  ...
  logAudit(c, {
    ...
    id_record: data.id,
    data_baru: data,
    deskripsi: `Mutasi ${body.tipe} asset ${asset2.kode_asset}`
  });
...
mutasi.put("/:id/approve", async (c) => {
  ...
  const user = c.get("user");
  const { id } = c.req.param();
  const { data: mutasi2 } = await supabase2.from("asset_mutasi").select("*, asset(*)").eq("id", id).single();
  if (!mutasi2)
    return notFound(c, "Mutasi tidak ditemukan");
  if (mutasi2.status !== "diajukan")
    return validationError(c, "Mutasi sudah diproses");
  const updateData = {
    status: "disetujui",
    approved_by: user.id,
    approved_at: (/* @__PURE__ */ new Date()).toISOString()
  };
  const { data, error: error2 } = await supabase2.from("asset_mutasi").update(updateData).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  logAudit(c, {
    ...
    nama_tabel: "asset_mutasi",
    id_record: id,
    deskripsi: `Mutasi ${mutasi2.nomor_mutasi} disetujui`
  });
...
mutasi.put("/:id/reject", async (c) => {
  ...
  const { data: mutasi2 } = await supabase2.from("asset_mutasi").select("*").eq("id", id).single();
  if (!mutasi2)
    return notFound(c, "Mutasi tidak ditemukan");
  const { data, error: error2 } = await supabase2.from("asset_mutasi").update({
    status: "ditolak",
  ...
  logAudit(c, {
    ...
    nama_tabel: "asset_mutasi",
    id_record: id,
    deskripsi: `Mutasi ${mutasi2.nomor_mutasi} ditolak`
  });
...
mutasi.put("/:id/terima", async (c) => {
  ...
  const { id } = c.req.param();
  const { catatan } = await c.req.json();
  const { data: mutasiData } = await supabase2.from("asset_mutasi").select("*").eq("id", id).single();
  if (!mutasiData)
    return notFound(c, "Mutasi tidak ditemukan");
  await supabase2.from("asset_mutasi").update({
    status: "selesai",
    catatan_penerima: catatan,
    diterima_oleh: user.id,
    diterima_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("id", id);
  const updateAsset = {};
  if (mutasiData.id_cabang_tujuan)
    updateAsset.id_cabang = mutasiData.id_cabang_tujuan;
  if (mutasiData.id_gudang_tujuan)
    updateAsset.id_gudang = mutasiData.id_gudang_tujuan;
  if (mutasiData.id_lokasi_tujuan)
    updateAsset.id_lokasi = mutasiData.id_lokasi_tujuan;
  await supabase2.from("asset").update(updateAsset).eq("id", mutasiData.id_asset);
  return success(c, null, "Penerimaan asset berhasil dikonfirmasi");
});
var mutasi_default = mutasi;

// src/api/v1/asset/stock-opname.ts
var app8 = new Hono2();
var stock_opname_default = app8;

// src/api/v1/asset/penyusutan.ts
var app9 = new Hono2();
var penyusutan_default = app9;

// src/api/v1/asset/penghapusan.ts
var app10 = new Hono2();
var penghapusan_default = app10;

// src/api/v1/maintenance/ticket.ts
var ticket = new Hono2();
...
ticket.get("/", async (c) => {
  ...
  let query = supabase2.from("maintenance_ticket").select("*, cabang!inner(nama), asset(nama, kode_asset), jenis_maintenance(nama), teknisi:users!id_teknisi(nama), vendor(nama)", { count: "exact" });
  if (["KCB", "PCB"].includes(roleCode) && user.id_cabang) {
    query = query.eq("id_cabang", user.id_cabang);
  }
  if (status)
...
ticket.get("/:id", async (c) => {
  const supabase2 = getSupabase(c.env);
  const { id } = c.req.param();
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").select("*, cabang(*), asset(*), jenis_maintenance(*), teknisi:users!id_teknisi(*), vendor(*), maintenance_foto(*), maintenance_tracking(*)").eq("id", id).single();
  if (error2 || !data)
    return notFound(c, "Ticket tidak ditemukan");
  return success(c, data);
});
ticket.post("/", async (c) => {
  const supabase2 = getSupabase(c.env);
  ...
  const tahun = (/* @__PURE__ */ new Date()).getFullYear();
  const { count } = await supabase2.from("maintenance_ticket").select("*", { count: "exact", head: true }).like("nomor_ticket", `TKT-${tahun}-%`);
  const nomorTicket = generateNomorTicket(tahun, (count || 0) + 1);
  const slaHours = SLA_HOURS[body.prioritas] || 48;
  const slaDeadline = new Date(Date.now() + slaHours * 60 * 60 * 1e3).toISOString();
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").insert({
  ...
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: data.id,
    status_dari: null,
...
ticket.put("/:id/approve", async (c) => {
  ...
  const user = c.get("user");
  const { id } = c.req.param();
  const { data: existing } = await supabase2.from("maintenance_ticket").select("*").eq("id", id).single();
  if (!existing)
    return notFound(c, "Ticket tidak ditemukan");
  if (existing.status !== MAINTENANCE_STATUS.DIBUAT)
    return validationError(c, "Status ticket tidak valid");
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").update({ status: MAINTENANCE_STATUS.DISETUJUI, approved_by: user.id, approved_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: id,
    status_dari: existing.status,
    status_ke: MAINTENANCE_STATUS.DISETUJUI,
    keterangan: "Ticket disetujui",
    user_id: user.id
  });
...
ticket.put("/:id/assign", async (c) => {
  ...
  if (!existing)
    return notFound(c, "Ticket tidak ditemukan");
  const update = { status: MAINTENANCE_STATUS.DITUGASKAN };
  if (id_teknisi)
    update.id_teknisi = id_teknisi;
  if (id_vendor)
    update.id_vendor = id_vendor;
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").update(update).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: id,
    status_dari: existing.status,
    status_ke: MAINTENANCE_STATUS.DITUGASKAN,
    keterangan: catatan || "Ditugaskan ke teknisi/vendor",
    user_id: user.id
  });
...
ticket.put("/:id/start", async (c) => {
  ...
  const user = c.get("user");
  const { id } = c.req.param();
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").update({ status: MAINTENANCE_STATUS.DIKERJAKAN, tanggal_mulai: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: id,
    status_dari: MAINTENANCE_STATUS.DITUGASKAN,
    status_ke: MAINTENANCE_STATUS.DIKERJAKAN,
    keterangan: "Pengerjaan dimulai",
    user_id: user.id
  });
...
ticket.put("/:id/complete", async (c) => {
  ...
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").update({
  ...
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: id,
    status_dari: MAINTENANCE_STATUS.DIKERJAKAN,
    status_ke: MAINTENANCE_STATUS.SELESAI,
    keterangan: catatan || "Perbaikan selesai",
    user_id: user.id
  });
...
ticket.put("/:id/verify", async (c) => {
  ...
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: id,
    status_dari: MAINTENANCE_STATUS.SELESAI,
    status_ke: newStatus,
...
ticket.put("/:id/close", async (c) => {
  ...
  if (!existing)
    return notFound(c, "Ticket tidak ditemukan");
  const { data, error: error2 } = await supabase2.from("maintenance_ticket").update({ status: MAINTENANCE_STATUS.CLOSED, closed_by: user.id, closed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("maintenance_tracking").insert({
    id_ticket: id,
    status_dari: existing.status,
    status_ke: MAINTENANCE_STATUS.CLOSED,
    keterangan: "Ticket ditutup",
    user_id: user.id
  });
  if (existing.id_asset) {
    await supabase2.from("asset").update({ status: "aktif" }).eq("id", existing.id_asset);
    await supabase2.from("asset_history").insert({
      id_asset: existing.id_asset,
      tipe_event: "status_change",
...
// src/api/v1/maintenance/preventive.ts
var app11 = new Hono2();
var preventive_default = app11;

// src/api/v1/maintenance/sla.ts
var app12 = new Hono2();
var sla_default = app12;

// src/api/v1/procurement/purchase-request.ts
var pr = new Hono2();
...
pr.get("/", async (c) => {
  ...
  let query = supabase2.from("purchase_request").select("*, cabang!inner(nama), creator:users!created_by(nama), purchase_request_item(*)", { count: "exact" });
  if (["KCB", "PCB"].includes(roleCode) && user.id_cabang) {
    query = query.eq("id_cabang", user.id_cabang);
  }
  if (status)
...
pr.get("/:id", async (c) => {
  const supabase2 = getSupabase(c.env);
  const { id } = c.req.param();
  const { data, error: error2 } = await supabase2.from("purchase_request").select("*, cabang(*), gudang(*), creator:users!created_by(*), purchase_request_item(*), purchase_request_approval(*, approved_by:users!approved_by(*))").eq("id", id).single();
  if (error2 || !data)
    return notFound(c, "PR tidak ditemukan");
  return success(c, data);
});
pr.post("/", async (c) => {
  const supabase2 = getSupabase(c.env);
  ...
  const tahun = (/* @__PURE__ */ new Date()).getFullYear();
  const { count } = await supabase2.from("purchase_request").select("*", { count: "exact", head: true }).like("nomor_pr", `PR-${tahun}-%`);
  ...
  const { data: prData, error: prError } = await supabase2.from("purchase_request").insert({
    nomor_pr: nomorPR,
    judul: body.judul,
    deskripsi: body.deskripsi,
    id_cabang: body.id_cabang,
  ...
  if (prError)
    return validationError(c, prError.message);
  ...
  const { error: itemsError } = await supabase2.from("purchase_request_item").insert(prItems);
  if (itemsError)
    return validationError(c, itemsError.message);
  logAudit(c, {
    tipe_aksi: AUDIT_ACTION.INSERT,
    modul: MODUL.PROCUREMENT,
    nama_tabel: "purchase_request",
    id_record: prData.id,
    data_baru: prData,
    deskripsi: `PR ${nomorPR} dibuat`
  });
...
pr.put("/:id/submit", async (c) => {
  const supabase2 = getSupabase(c.env);
  const { id } = c.req.param();
  const { data, error: error2 } = await supabase2.from("purchase_request").update({ status: PR_STATUS.DIAJUKAN }).eq("id", id).eq("status", PR_STATUS.DRAFT).select("*").single();
  if (error2 || !data)
    return validationError(c, "PR tidak ditemukan atau sudah diajukan");
...
pr.put("/:id/approve-kacab", async (c) => {
  ...
  const { id } = c.req.param();
  const { catatan } = await c.req.json();
  const { data: existing } = await supabase2.from("purchase_request").select("*").eq("id", id).single();
  if (!existing)
    return notFound(c, "PR tidak ditemukan");
  if (existing.status !== PR_STATUS.DIAJUKAN)
    return validationError(c, "Status PR tidak valid");
  const { data, error: error2 } = await supabase2.from("purchase_request").update({ status: PR_STATUS.DISETUJUI_KACAB }).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("purchase_request_approval").insert({
    id_pr: id,
    tahap: "kacab",
    status: "approved",
    catatan,
...
pr.put("/:id/approve-headga", async (c) => {
  ...
  if (!existing)
    return notFound(c, "PR tidak ditemukan");
  if (existing.status !== PR_STATUS.DISETUJUI_KACAB)
    return validationError(c, "PR harus disetujui Kacab terlebih dahulu");
  const { data, error: error2 } = await supabase2.from("purchase_request").update({ status: PR_STATUS.DISETUJUI_HGA }).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("purchase_request_approval").insert({
    id_pr: id,
...
pr.put("/:id/reject", async (c) => {
  ...
  const { data, error: error2 } = await supabase2.from("purchase_request").update({ status: PR_STATUS.DITOLAK }).eq("id", id).select("*").single();
  if (error2)
    return validationError(c, error2.message);
  await supabase2.from("purchase_request_approval").insert({
    id_pr: id,
...
var purchase_request_default = pr;

// src/api/v1/procurement/purchase-order.ts
var app13 = new Hono2();
var purchase_order_default = app13;

// src/api/v1/procurement/penerimaan.ts
var app14 = new Hono2();
var penerimaan_default = app14;

// src/api/v1/vehicle/booking.ts
var app15 = new Hono2();
var booking_default = app15;

// src/api/v1/building/checklist.ts
var app16 = new Hono2();
var checklist_default = app16;

// src/api/v1/building/issue.ts
var app17 = new Hono2();
var issue_default = app17;

// src/api/v1/auth.ts
var app18 = new Hono2();
var auth_default = app18;

// src/api/v1/dashboard/head-ga.ts
var app19 = new Hono2();
var head_ga_default = app19;

// src/api/v1/audit/index.ts
var app20 = new Hono2();
var audit_default = app20;

// src/api/router.ts
var router = new Hono2();
router.get("/health", (c) => c.json({ status: "ok", timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
router.route("/auth", auth_default);
router.route("/master/cabang", cabang_default);
router.route("/master/gudang", gudang_default);
router.route("/master/user", user_default);
router.route("/master/vendor", vendor_default);
router.route("/master/kategori-asset", kategori_asset_default);
router.route("/master/lokasi-asset", lokasi_asset_default);
router.route("/master/jenis-maintenance", jenis_maintenance_default);
router.route("/asset", asset_default);
router.route("/asset/mutasi", mutasi_default);
router.route("/asset/stock-opname", stock_opname_default);
router.route("/asset/penyusutan", penyusutan_default);
router.route("/asset/penghapusan", penghapusan_default);
router.route("/maintenance/ticket", ticket_default);
router.route("/maintenance/preventive", preventive_default);
router.route("/maintenance/sla", sla_default);
router.route("/procurement/pr", purchase_request_default);
router.route("/procurement/po", purchase_order_default);
router.route("/procurement/penerimaan", penerimaan_default);
router.route("/vehicle/booking", booking_default);
router.route("/building/checklist", checklist_default);
router.route("/building/issue", issue_default);
router.route("/dashboard/head-ga", head_ga_default);
router.route("/audit", audit_default);
...
app21.use("*", errorHandler2);
app21.route("/api/v1", router_default);
app21.get("/", (c) => c.json({
...
// .wrangler/tmp/bundle-4UJ6bo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-4UJ6bo/middleware-loader.entry.ts
